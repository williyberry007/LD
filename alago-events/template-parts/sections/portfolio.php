...
$kicker = get_theme_mod( 'alago_portfolio_kicker', __( 'Real Celebrations', 'alago-events' ) );
$title  = get_theme_mod( 'alago_portfolio_title', __( 'Selected Work', 'alago-events' ) );
$intro  = get_theme_mod( 'alago_portfolio_intro', '' );
$count  = (int) get_theme_mod( 'alago_portfolio_count', 6 );
	...
	<div class="ae-container">
			...
			<?php else : ?>
				<?php for ( $i = 1; $i <= max( 3, $count ); $i++ ) : ?>
					<a class="ae-portfolio__item ae-reveal" href="#">
						...
						<div class="ae-portfolio__caption">
							<span><?php esc_html_e( 'Destination Wedding', 'alago-events' ); ?></span>
							<h3><?php printf( esc_html__( 'Celebration %d', 'alago-events' ), (int) $i ); ?></h3>
						</div>
					</a>
				<?php endfor; ?>
			<?php endif; ?>
		...
		<?php if ( $items->have_posts() || post_type_exists( 'ae_portfolio' ) ) : ?>
			<div style="text-align:center;margin-top:3rem;">
				<a class="ae-btn" href="<?php echo esc_url( get_post_type_archive_link( 'ae_portfolio' ) ?: '#' ); ?>"><?php esc_html_e( 'View Full Portfolio', 'alago-events' ); ?></a>
			</div>
		<?php endif; ?>
	</div>
</section>
