...
<?php while ( have_posts() ) : the_post(); ?>
	...
	<div class="ae-page-header" <?php echo has_post_thumbnail() ? 'style="background-image:linear-gradient(rgba(20,20,18,.6),rgba(20,20,18,.6)),url(' . esc_url( get_the_post_thumbnail_url( null, 'full' ) ) . ');background-size:cover;background-position:center;"' : ''; ?>>
		<div class="ae-container">
			<span class="ae-breadcrumb"><?php echo esc_html( get_post_type() === 'post' ? __( 'Journal', 'alago-events' ) : get_post_type_object( get_post_type() )->labels->singular_name ); ?></span>
			<h1><?php the_title(); ?></h1>
				...
				<main id="primary" class="site-main">
					<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						<div class="entry-content">
							...
							wp_link_pages(
								array(
									'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'alago-events' ),
									'after'  => '</div>',
								)
							);
							?>
						</div>
						<footer class="entry-footer"><?php alago_events_entry_footer(); ?></footer>
					</article>

					<?php
					the_post_navigation(
						array(
							'prev_text' => '<span class="ae-post-meta">' . esc_html__( 'Previous', 'alago-events' ) . '</span> %title',
							'next_text' => '<span class="ae-post-meta">' . esc_html__( 'Next', 'alago-events' ) . '</span> %title',
						)
					);
