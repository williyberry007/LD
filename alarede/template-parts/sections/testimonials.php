...
$bg     = get_theme_mod( 'alarede_testimonials_bg', '' );
$kicker = get_theme_mod( 'alarede_testimonials_kicker', __( 'Kind Words', 'alarede' ) );
$title  = get_theme_mod( 'alarede_testimonials_title', __( 'Love Notes From Our Couples', 'alarede' ) );
...
$fallback = array(
	array( __( 'They turned our wildest dreams into a flawless reality. Every detail was perfect, and we were able to simply enjoy our day.', 'alarede' ), __( 'Sofia & James', 'alarede' ), __( 'Coastal Villa', 'alarede' ) ),
	array( __( 'Calm, creative and utterly professional. We could not have imagined a more beautiful celebration.', 'alarede' ), __( 'Amara & Daniel', 'alarede' ), __( 'Mountain Estate', 'alarede' ) ),
	array( __( 'From the first call to the last dance, we felt completely cared for. Truly the best decision we made.', 'alarede' ), __( 'Elena & Marco', 'alarede' ), __( 'Seaside Garden', 'alarede' ) ),
);
?>
			...
			<?php if ( $index > 1 ) : ?>
				<div class="ae-testimonials__dots">
					<?php for ( $d = 0; $d < $index; $d++ ) : ?>
						<button class="<?php echo 0 === $d ? 'is-active' : ''; ?>" data-slide="<?php echo (int) $d; ?>" aria-label="<?php printf( esc_attr__( 'Testimonial %d', 'alarede' ), $d + 1 ); ?>"></button>
					<?php endfor; ?>
				</div>
			<?php endif; ?>
		</div>
