...
- **Jobs** — Careers openings with full details (post content), an employment
  detail line and an editable apply link (URL or email). Set a Featured Image
  for the job's banner and a list thumbnail, and assign **Job Categories** to
  enable the filter buttons on the Careers page. Each job lists with "Full
  Details" + "Apply" buttons and has its own page.
